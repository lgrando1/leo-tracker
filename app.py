...
def inicializar_banco():
    ...
    # NOVA TABELA PARA PRESSÃO ARTERIAL
    executar_sql("CREATE TABLE IF NOT EXISTS public.pressao (id SERIAL PRIMARY KEY, data DATE, sis INTEGER, dia INTEGER, bpm INTEGER);")
...
# ABAS (Nova aba SAÚDE adicionada)
tab_add, tab_hist, tab_peso, tab_saude, tab_rel, tab_admin = st.tabs(["➕ Inserir", "📜 Diário", "⚖️ Peso", "❤️ Saúde", "📄 Relatórios", "⚙️ Metas"])
...
# ABA 4: SAÚDE (PRESSÃO) - NOVA E OTIMIZADA
with tab_saude:
    st.header("❤️ Monitoramento de Pressão")
    st.info("Preencha os dados abaixo. A página não irá atualizar enquanto você digita.")
    
    with st.form("form_pressao"):
        col_dt = st.date_input("Data da Medição:", value=data_hoje)
        
        c1, c2, c3 = st.columns(3)
        sis = c1.number_input("Sistólica (Alta):", min_value=0, max_value=300, value=120)
        dia = c2.number_input("Diastólica (Baixa):", min_value=0, max_value=200, value=80)
        bpm = c3.number_input("Batimentos (BPM):", min_value=0, max_value=200, value=75)
        
        if st.form_submit_button("💾 Salvar Pressão"):
            executar_sql("INSERT INTO public.pressao (data, sis, dia, bpm) VALUES (%s, %s, %s, %s)", (col_dt, sis, dia, bpm))
            st.success("Dados vitais registrados!")
            st.rerun()

    # Histórico de Pressão
    st.divider()
    df_press = executar_sql("SELECT * FROM public.pressao ORDER BY data DESC LIMIT 10", is_select=True)
    if not df_press.empty:
        st.write("#### Últimas Medições")
        for i, row in df_press.iterrows():
            st.markdown(f"**{pd.to_datetime(row['data']).strftime('%d/%m')}** | ❤️ **{row['sis']}x{row['dia']}** mmHg | 💓 {row['bpm']} bpm")
            st.divider()
...
with tab_rel:
    ...
    if st.button("🔍 Gerar Arquivos"):
        ...
        if not df_cons_rel.empty:
            st.download_button("📥 Excel (.xlsx)", gerar_excel(df_cons_rel, df_peso_rel, d_inicio, d_fim), f"Relatorio_{d_inicio}.xlsx")
            try: st.download_button("📥 PDF (.pdf)", gerar_pdf(df_cons_rel, df_peso_rel, d_inicio, d_fim), f"Resumo_{d_inicio}.pdf")
...
st.caption(f"Leo Tracker Pro v3.5 | Form Optimization")
